...
{
	// set up EPWM-A
	GPIO_SetupPinOptions(MOTOR2_EPWM_A_GPIO, GPIO_OUTPUT, GPIO_PULLUP);
	GPIO_SetupPinMux(MOTOR2_EPWM_A_GPIO, 0, MOTOR2_EPWM_A_MUX);

	// set up EPWM-B
	GPIO_SetupPinOptions(MOTOR2_EPWM_B_GPIO, GPIO_OUTPUT, GPIO_PULLUP);
	GPIO_SetupPinMux(MOTOR2_EPWM_B_GPIO, 0, MOTOR2_EPWM_B_MUX);

	// set up EPWM-C
	GPIO_SetupPinOptions(MOTOR2_EPWM_C_GPIO, GPIO_OUTPUT, GPIO_PULLUP);
	GPIO_SetupPinMux(MOTOR2_EPWM_C_GPIO, 0, MOTOR2_EPWM_C_MUX);
...
void InitMotor1EQepGpio(void)
{
	...
	// Set up QEP-I	// Disable I input, code must work without
...
{
	// Set up QEP-A
	GPIO_SetupPinOptions(MOTOR2_QEPA_GPIO, GPIO_INPUT, GPIO_SYNC);
	GPIO_SetupPinMux(MOTOR2_QEPA_GPIO,0,MOTOR2_QEPA_MUX);

	// Set up QEP-B
	GPIO_SetupPinOptions(MOTOR2_QEPB_GPIO, GPIO_INPUT, GPIO_SYNC);
	GPIO_SetupPinMux(MOTOR2_QEPB_GPIO,0,MOTOR2_QEPB_MUX);

	// Set up QEP-I		//Motor 2 has no I-input available
	//GPIO_SetupPinOptions(MOTOR2_QEPI_GPIO, GPIO_INPUT, GPIO_SYNC);
	//GPIO_SetupPinMux(MOTOR2_QEPI_GPIO,0,MOTOR2_QEPI_MUX);
...
{
	// set up SDI pin
	GPIO_SetupPinOptions(MOTOR2_SDI_GPIO, GPIO_INPUT, GPIO_ASYNC);
	GPIO_SetupPinMux(MOTOR2_SDI_GPIO,0,MOTOR2_SDI_MUX);

	// set up SDO pin
	GPIO_SetupPinOptions(MOTOR2_SDO_GPIO, GPIO_INPUT, GPIO_ASYNC);
	GPIO_SetupPinMux(MOTOR2_SDO_GPIO,0,MOTOR2_SDO_MUX);

	// set up CLK pin
	GPIO_SetupPinOptions(MOTOR2_CLK_GPIO, GPIO_INPUT, GPIO_ASYNC);
	GPIO_SetupPinMux(MOTOR2_CLK_GPIO,0,MOTOR2_CLK_MUX);

	// set up SCS pin
	GPIO_SetupPinOptions(MOTOR2_SCS_GPIO, GPIO_OUTPUT, GPIO_ASYNC);
	GPIO_SetupPinMux(MOTOR2_SCS_GPIO,0,MOTOR2_SCS_MUX);
...
{
	// Configure as Output : Motor 2 - EN-GATE
	GPIO_WritePin(MOTOR2_EN_GATE_GPIO, 0);  // Disable EN_GATE
	GPIO_SetupPinOptions(MOTOR2_EN_GATE_GPIO, GPIO_OUTPUT, GPIO_ASYNC);
	GPIO_SetupPinMux(MOTOR2_EN_GATE_GPIO, 0, MOTOR2_EN_GATE_MUX);

#if (DRV_MOTOR1 == DRV8301)
	// Configure as Output : Motor 1 - DC-CAL for DRV8301
	GPIO_WritePin(MOTOR2_DC_CAL_GPIO, 1);  // Set DC-CAL to 1 to disable amplifiers
	GPIO_SetupPinOptions(MOTOR2_DC_CAL_GPIO, GPIO_OUTPUT, GPIO_ASYNC);
	GPIO_SetupPinMux(MOTOR2_DC_CAL_GPIO, 0, MOTOR2_DC_CAL_MUX);
#else
	// Configure as Output : Motor 1 - WAKE for DRV8305
	GPIO_WritePin(MOTOR2_WAKE_GPIO, 0);  // Set WAKE to 0 (default state)
	GPIO_SetupPinOptions(MOTOR2_WAKE_GPIO, GPIO_OUTPUT, GPIO_ASYNC);
	GPIO_SetupPinMux(MOTOR2_WAKE_GPIO, 0, MOTOR2_WAKE_MUX);
#endif
}

// ****************************************************************************
// ****************************************************************************
// CPU SYSTEM CONTROL Configuration
// ****************************************************************************
// ****************************************************************************
...
void PWM_1ch_UpDwnCnt_CNF(volatile struct EPWM_REGS * ePWM, Uint16 period, int16 db)
{
	...
	//EDITED
#ifndef _FLASH
	ePWM->DBRED.all = (Uint16)db;
	ePWM->DBFED.all = (Uint16)db;
#else
	ePWM->DBRED = (Uint16)db;
	ePWM->DBFED = (Uint16)db;
#endif
	//EDITED_END
	EDIS;
}
