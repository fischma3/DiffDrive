...
/*-----------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------*/
typedef struct {
	float32 ElecTheta;        // Output: Motor Electrical angle (Q24)
...
{																						\
	...
	(*eQEP).QPOSMAX = 4294964000; /*biggest multiple of full turn */					\
}

#define QEP_MACRO(eQEP, motorQEP)														\
{																						\
	/* Check the rotational direction */												\
	motorQEP.DirectionQep = (*eQEP).QEPSTS.bit.QDF;										\
																						\
    /* Check the position counter for EQEP1 */											\
	motorQEP.RawTheta = ((*eQEP).QPOSCNT + motorQEP.CalibratedAngle) % 4000;			\
																						\
    if (motorQEP.RawTheta < 0)															\
		motorQEP.RawTheta = motorQEP.RawTheta + (*eQEP).QPOSMAX;						\
    else if (motorQEP.RawTheta > (*eQEP).QPOSMAX)										\
		motorQEP.RawTheta = motorQEP.RawTheta - (*eQEP).QPOSMAX;						\
																						\
    /* Compute the mechanical angle */													\
	motorQEP.MechTheta= motorQEP.MechScaler*motorQEP.RawTheta;							\
    /* Compute the electrical angle  */													\
	motorQEP.ElecTheta = (motorQEP.PolePairs*motorQEP.MechTheta) -floor(motorQEP.PolePairs*motorQEP.MechTheta); /* Q24 = Q0*Q24 */ \
																						\
    /* Check an index occurrence*/														\
	/*if ((*eQEP).QFLG.bit.IEL == 1)	*/													\
	/*{*/																					\
		/*motorQEP.IndexSyncFlag = 0x00F0;*/												\
		/*motorQEP.QepCountIndex = (*eQEP).QPOSILAT;*/										\
		/*(*eQEP).QCLR.bit.IEL = 1; *//* Clear interrupt flag*/								\
	/*}*/																					\
	/*Set the compare register for next execution*/										\
	(*eQEP).QPOSCMP = (*eQEP).QPOSCNT;													\
    ...
    {																					\
        ...
        {	/* Capture Counter overflowed, hence do no compute speed*/					\
			...
			motorQEP.QepPeriod = (*eQEP).QCPRDLAT;										\
    }                                                                                   \
}
